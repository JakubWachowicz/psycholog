...
class MessagesConroller extends GetxController {
  ...
  Future<int> getUnreadMessageCount(String docId) async {
    var from_messages = await db
        ...
        .where("isRead", isEqualTo: "True").limit(state.messageLimit.value)
        .get();
  ...
  asyncLoadAllData() async {
    var from_messages = await db
        ...
        .withConverter(
            fromFirestore: Msg.fromFirestore,
            toFirestore: (Msg msg, options) => msg.toFirestore())
        .where("from_uid", isEqualTo: token).limit(state.messageLimit.value)
        .get();
    var to_messages = await db
        .collection("messages")
        .withConverter(
            fromFirestore: Msg.fromFirestore,
            toFirestore: (Msg msg, options) => msg.toFirestore())
        .where("to_uid", isEqualTo: token).limit(state.messageLimit.value)
        .get();
